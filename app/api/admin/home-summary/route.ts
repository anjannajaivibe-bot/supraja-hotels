...
export async function GET(request:NextRequest){
 ...
 const[shiftRes,staffRes,attendanceRes,complaintRes,maintenanceRes]=await Promise.all([
 ...
 if(![shiftRes,staffRes,attendanceRes,complaintRes,maintenanceRes].every(r=>r.ok))return NextResponse.json({error:"Unable to load today's operational summary."},{status:500});

 // Opening cash was added after the checklist table was already live. During a staged
 // database rollout, gracefully fall back to the older checklist projection so the
 // Master Home never becomes unavailable just because the new column is not present yet.
 ...
 if(!checklistRes.ok){
  openingCashSupported=false;
  checklistRes=await supabaseRequest(`?select=hotel_id,checklist_type,item_key,is_completed&hotel_id=in.(${inFilter})&checklist_date=eq.${date}`,{},"hotel_checklist_entries");
 }
 ...
 return NextResponse.json({date,hotels:summaries,openingCashSupported});
}
