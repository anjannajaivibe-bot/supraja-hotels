...
type Staff={id:string;hotel_id:string};type Attendance={hotel_id:string;status:string};type Checklist={hotel_id:string;checklist_type:string;is_completed:boolean};type Issue={hotel_id:string;priority?:string};
...
export async function GET(request:NextRequest){
 ...
 const summaries=hotels.map(hotel=>{const shift=shifts.find(s=>s.hotel_id===hotel.id)??null,hotelStaff=staff.filter(s=>s.hotel_id===hotel.id).length,hotelAttendance=attendance.filter(a=>a.hotel_id===hotel.id).length,count=(type:string)=>checklists.filter(c=>c.hotel_id===hotel.id&&c.checklist_type===type&&c.is_completed).length,openComplaints=complaints.filter(i=>i.hotel_id===hotel.id),openMaintenance=maintenance.filter(i=>i.hotel_id===hotel.id);return{id:hotel.id,name:hotel.name,code:hotel.code,onDuty:shift?{employeeName:shift.display_name,startedAt:shift.started_at,shiftId:shift.id,shiftType:shift.shift_type,isLate:!!shift.is_late,lateMinutes:shift.late_minutes||0,lateReason:shift.late_reason||null}:null,cleaningStaff:{total:hotelStaff,marked:hotelAttendance},checklists:{shiftStart:{done:count("shift_start"),total:6},daily:{done:count("daily"),total:8},shiftEnd:{done:count("shift_end"),total:8}},issues:{complaints:openComplaints.length,maintenance:openMaintenance.length,critical:[...openComplaints,...openMaintenance].filter(i=>i.priority==="critical").length}}});
