...
type Checklist={hotel_id:string;checklist_type:string;item_key:string;is_completed:boolean;opening_cash_amount?:number|null;shift_id?:string|null;scope_key?:string|null};
...
export async function GET(request:NextRequest){
 ...
 let checklistRes=await supabaseRequest(`?select=hotel_id,checklist_type,item_key,is_completed,opening_cash_amount,shift_id,scope_key&hotel_id=in.(${inFilter})&checklist_date=eq.${date}`,{},"hotel_checklist_entries");
 let openingCashSupported=true;
 if(!checklistRes.ok){openingCashSupported=false;checklistRes=await supabaseRequest(`?select=hotel_id,checklist_type,item_key,is_completed,shift_id,scope_key&hotel_id=in.(${inFilter})&checklist_date=eq.${date}`,{},"hotel_checklist_entries")}
 ...
 const summaries=hotels.map(hotel=>{
  const shift=shifts.find(s=>s.hotel_id===hotel.id)??null,hotelStaff=staff.filter(s=>s.hotel_id===hotel.id).length,hotelAttendance=attendance.filter(a=>a.hotel_id===hotel.id).length,hotelChecklists=checklists.filter(c=>c.hotel_id===hotel.id),currentShiftChecklists=shift?hotelChecklists.filter(c=>c.shift_id===shift.id||c.scope_key===`shift:${shift.id}`):[],dailyChecklists=hotelChecklists.filter(c=>c.checklist_type==="daily"),count=(type:string)=>(type==="daily"?dailyChecklists:currentShiftChecklists).filter(c=>c.checklist_type===type&&c.is_completed).length,openingCashEntry=currentShiftChecklists.find(c=>c.checklist_type==="shift_start"&&c.item_key==="cash_opening_verified"&&c.is_completed),openComplaints=complaints.filter(i=>i.hotel_id===hotel.id),openMaintenance=maintenance.filter(i=>i.hotel_id===hotel.id);
