...
export const metadata: Metadata = {
  ...
  description: "Compare Supraja Hotels in Madhapur and Chandanagar. View real hotel photos, locations and contact each property directly for current room availability and rates.",
  alternates: { canonical: "/hotels" },
  openGraph: { title: "Hotels in Hyderabad | Supraja Hotels", description: "Choose a Supraja Hotel in Madhapur or Chandanagar and contact the property directly.", url: "/hotels", siteName: "Supraja Hotels", images: [{ url: "/images/social/supraja-hotels-og.jpg", width: 1200, height: 630, alt: "Supraja Hotels in Hyderabad" }], locale: "en_IN", type: "website" },
};

const copy: Record<string, string> = {
  "supraja-cyber-view": "Best suited for Madhapur and HITEC City visits, opposite Shilpakala Vedika and convenient for HITEC City Metro, Shilparamam, Mindspace, Kondapur and Gachibowli.",
  "supraja-residency": "A comfortable Chandanagar option near Gangaram with convenient access to Lingampally Railway Station, BHEL, Serilingampally and Miyapur.",
  "supraja-lodge": "A budget-friendly Chandanagar stay for short visits near Lingampally Railway Station, Gangaram, BHEL and Miyapur.",
};
  ...
  return <main className="bg-white pb-20 text-slate-900 md:pb-0">
    ...
    <section className="relative isolate overflow-hidden bg-slate-950 text-white">
      ...
      <div className="container-custom relative z-10 px-5 py-20 sm:px-6 lg:py-24">
        <div className="max-w-4xl"><p className="text-sm font-bold uppercase tracking-[0.24em] text-amber-300">Choose Your Stay</p><h1 className="mt-4 text-4xl font-bold leading-[1.05] tracking-tight sm:text-5xl md:text-6xl">Hotels in Hyderabad, Chosen by Location</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-white/88">Compare our Madhapur and Chandanagar properties visually, choose the location that fits your visit, then contact the hotel directly for current availability and rates.</p><div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-white/90">{["Real property photos","Direct hotel contact","Madhapur & Chandanagar","Call & WhatsApp support"].map((item)=><span key={item} className="flex items-center gap-2"><CheckCircle2 size={17} className="text-amber-300"/>{item}</span>)}</div></div>
      </div>
    </section>

    <section className="bg-[#f7f5f1] px-4 py-16 lg:py-24"><div className="container-custom">
      <div className="max-w-4xl"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-700">Our Hotels</p><h2 className="mt-3 text-3xl font-bold tracking-tight md:text-5xl">See the Property. Compare the Location. Choose with Confidence.</h2></div>
      <div className="mt-10 grid gap-7 lg:grid-cols-3">
        {hotels.map((hotel)=><article key={hotel.slug} className="group overflow-hidden rounded-[1.75rem] bg-white shadow-[0_18px_50px_rgba(15,23,42,.09)] transition hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(15,23,42,.14)]">
          ...
          <div className="p-6"><p className="leading-7 text-slate-600">{copy[hotel.slug]}</p><div className="mt-5 flex flex-wrap gap-2">{hotel.seo.targetLocations.slice(0,3).map((x)=><span key={x} className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700">{x}</span>)}</div><div className="mt-6 grid grid-cols-2 gap-2"><a href={`tel:+91${hotel.phone}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-slate-300 text-sm font-bold"><Phone size={16}/>Call</a><a href={`https://wa.me/91${hotel.whatsapp}?text=Hi%20I%20would%20like%20to%20check%20room%20availability%20at%20${encodeURIComponent(hotel.name)}`} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-green-600 text-sm font-bold text-white"><MessageCircle size={16}/>WhatsApp</a></div><Link href={`/hotels/${hotel.slug}`} className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-blue-800 hover:underline">See rooms & hotel details <ArrowRight size={15}/></Link></div>
        </article>)}
      </div>
    </div></section>

    <section className="bg-white px-4 py-16 lg:py-20"><div className="container-custom rounded-[2rem] bg-slate-950 p-8 text-white md:p-12"><div className="flex flex-col justify-between gap-7 md:flex-row md:items-center"><div><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-300">Need Help Choosing?</p><h2 className="mt-3 text-3xl font-bold">Tell Us Where You Need to Stay</h2><p className="mt-3 max-w-2xl text-slate-300">We can guide you to the most suitable Supraja Hotel based on Madhapur, HITEC City, Chandanagar, Lingampally, BHEL or Miyapur.</p></div><a href="https://wa.me/919550776161?text=Hi%20I%20need%20help%20choosing%20a%20Supraja%20Hotel" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-green-600 px-7 text-sm font-bold"><MessageCircle size={17}/>Ask on WhatsApp</a></div></div></section>
