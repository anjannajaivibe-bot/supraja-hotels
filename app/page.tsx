...
export default function HomePage() {
  ...
  return (
    <main className="bg-white text-slate-900 pb-20 md:pb-0">
      ...
      <section className="relative isolate flex min-h-[650px] items-end overflow-hidden bg-slate-950 text-white md:min-h-[760px] md:items-center" style={{ backgroundImage: "url('/media/home-hero/supraja-hotels-home-hero-poster.webp')", backgroundPosition: "center", backgroundSize: "cover" }}>
        <DeferredHeroVideo />
        <div className="absolute inset-0 z-10 bg-gradient-to-r from-slate-950/90 via-slate-950/48 to-transparent" aria-hidden="true" />
        <div className="absolute inset-0 z-10 bg-gradient-to-t from-slate-950/90 via-slate-950/15 to-slate-950/10" aria-hidden="true" />
        <div className="container-custom relative z-20 w-full px-5 pb-12 pt-32 sm:px-6 md:py-24">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.28em] text-amber-300 sm:text-sm">Supraja Hotels · Hyderabad</p>
            <h1 className="mt-5 text-4xl font-bold leading-[1.04] tracking-tight sm:text-5xl md:text-6xl lg:text-7xl">Stay Close to Where Hyderabad Takes You</h1>
            <p className="mt-6 max-w-2xl text-base leading-7 text-white/90 sm:text-lg sm:leading-8">Comfortable hotels in <strong>Madhapur near HITEC City</strong> and <strong>Chandanagar near Lingampally, BHEL and Miyapur</strong>. See the property, choose your location and contact the hotel directly.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="#choose-hotel" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-7 py-3 text-sm font-bold text-slate-950 shadow-xl transition hover:bg-amber-50">Choose Your Hotel <ArrowRight size={17} /></Link><a href="https://wa.me/919550776161?text=Hi%20I%20would%20like%20to%20check%20room%20availability%20at%20Supraja%20Hotels" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-green-600 px-7 py-3 text-sm font-bold text-white shadow-xl transition hover:bg-green-700"><MessageCircle size={18} />Check Availability</a></div>
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm font-medium text-white/90"><span className="flex items-center gap-2"><CheckCircle2 size={17} className="text-amber-300" />Direct hotel contact</span><span className="flex items-center gap-2"><CheckCircle2 size={17} className="text-amber-300" />Real property photos</span><span className="flex items-center gap-2"><CheckCircle2 size={17} className="text-amber-300" />Call & WhatsApp support</span></div>
          </div>
        </div>
      </section>

      <section id="choose-hotel" className="bg-[#f7f5f1] px-4 py-16 lg:py-24">
        <div className="container-custom">
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div className="max-w-3xl"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-700">Choose Your Stay</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 md:text-5xl">Three Hotels. Two Key Hyderabad Locations.</h2><p className="mt-5 max-w-2xl leading-8 text-slate-600">Start with where you need to be. Each property has a different location and stay style, with direct phone and WhatsApp support.</p></div><Link href="/gallery" className="inline-flex items-center gap-2 self-start text-sm font-bold text-blue-800 hover:underline lg:self-auto"><Camera size={18} />Explore all hotel photos</Link></div>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-3">
            {hotels.map((hotel) => <article key={hotel.slug} className="group overflow-hidden rounded-[1.75rem] bg-white shadow-[0_18px_50px_rgba(15,23,42,0.09)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(15,23,42,0.14)]">
              <Link href={`/hotels/${hotel.slug}`} className="relative block h-[320px] overflow-hidden bg-slate-100"><SmartImage src={hotel.images.hero} alt={hotel.seo.featuredImageAlt} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(max-width: 1024px) 100vw, 33vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/65 via-transparent to-transparent" /><div className="absolute inset-x-0 bottom-0 p-6 text-white"><p className="flex items-center gap-2 text-sm font-semibold text-white/90"><MapPin size={16} />{hotel.location}</p><h3 className="mt-2 text-2xl font-bold">{hotel.name}</h3></div></Link>
      ...
      <section className="bg-white px-4 py-16 lg:py-24"><div className="container-custom"><div className="grid items-center gap-10 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <div className="lg:pl-8"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-700">See Before You Stay</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 md:text-5xl">The Room Matters. So Does the Location.</h2><p className="mt-5 leading-8 text-slate-600">A hotel website should help you understand the stay before you call. Explore real property images, compare locations and choose the Supraja Hotel that fits your Hyderabad visit.</p><div className="mt-7 space-y-4">{["Madhapur option close to HITEC City and major IT destinations", "Chandanagar options for Lingampally, BHEL, Serilingampally and Miyapur", "Direct contact with each property for current availability and rates"].map((item) => <p key={item} className="flex gap-3 leading-7 text-slate-700"><CheckCircle2 className="mt-1 shrink-0 text-green-700" size={19} />{item}</p>)}</div><Link href="/gallery" className="mt-8 inline-flex items-center gap-2 rounded-full bg-slate-950 px-6 py-3 text-sm font-bold text-white transition hover:bg-slate-800"><Camera size={17} />View Hotel Gallery</Link></div>
      </div></div></section>

      <section className="bg-slate-950 px-4 py-10 text-white"><div className="container-custom grid gap-6 md:grid-cols-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-300">Book Direct</p><h2 className="mt-2 text-2xl font-bold">Talk to the Hotel, Not a Call Centre</h2></div><div className="flex items-start gap-3"><Phone className="mt-1 text-amber-300" size={22} /><div><p className="font-bold">Current Availability</p><p className="mt-1 text-sm leading-6 text-slate-300">Ask the property about rooms and current rates before travelling.</p></div></div><div className="flex items-start gap-3"><MessageCircle className="mt-1 text-amber-300" size={22} /><div><p className="font-bold">WhatsApp Support</p><p className="mt-1 text-sm leading-6 text-slate-300">Message the hotel directly when calling is not convenient.</p></div></div></div></section>

      <section className="bg-[#f7f5f1] px-4 py-16 lg:py-24"><div className="container-custom"><div className="mx-auto max-w-3xl text-center"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-700">Your Stay, Simplified</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 md:text-5xl">What You Can Expect</h2></div><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{featureCards.map((feature) => { const Icon = feature.icon; return <div key={feature.title} className="rounded-[1.5rem] bg-white p-7 shadow-sm"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-amber-50 text-amber-800"><Icon size={21} /></div><h3 className="mt-5 text-xl font-bold text-slate-950">{feature.title}</h3><p className="mt-3 leading-7 text-slate-600">{feature.description}</p></div>; })}</div></div></section>

      <AmenitiesSection />

      <section className="bg-white px-4 py-16 lg:py-24"><div className="container-custom"><div className="rounded-[2rem] bg-slate-950 p-8 text-white md:p-12"><div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-300">Ready to Stay?</p><h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">Choose Your Hotel and Check Availability</h2><p className="mt-4 max-w-2xl leading-8 text-slate-300">See the property first, choose the location that works for you and contact the hotel directly for current room availability and rates.</p></div><div className="flex flex-col gap-3 sm:flex-row lg:flex-col"><Link href="/hotels" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-7 py-3 text-sm font-bold text-slate-950">Choose a Hotel <ArrowRight size={16} /></Link><a href="https://wa.me/919550776161?text=Hi%20I%20would%20like%20to%20check%20room%20availability%20at%20Supraja%20Hotels" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-green-600 px-7 py-3 text-sm font-bold text-white"><MessageCircle size={17} />WhatsApp</a></div></div></div></div></section>
